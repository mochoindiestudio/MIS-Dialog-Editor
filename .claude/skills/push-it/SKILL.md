...
A repeatable release routine for this project, adapted from the Lucy project's `push-it` skill for a
UPM package repo (version lives in `package.json`, not `ProjectSettings.asset`). Run these steps in
order, every time this skill is invoked. Do not skip steps or silently reinterpret them — this touches
shared remote state.
...
Use AskUserQuestion. Do not assume — even when the scope looks obvious from `git status`.
...
Version lives in the package's `package.json` (`Packages/com.mochoindiestudio.node-dialog-system/package.json`)
as `"version": "X.Y.Z"` — this is the UPM package version, not a Unity `bundleVersion` (there is no
game/player build here).
...
- **Minor** (`Y += 1`, `Z` resets to `0`) — new features, new node types, new editor tooling, or
  otherwise substantial additions.
...
Confirm a remote exists first (`git remote -v`) — `origin` is `mochoindiestudio/MIS-Dialog-Editor` on
GitHub. Work happens on `development` per [[project_branching_workflow]] — push there (`git push`, add
`-u origin development` only if tracking isn't already set up), never push to `main` unless the user
explicitly asks for a PR/merge.
...
```
git tag -a vX.Y.Z -m "MIS Dialog System vX.Y.Z"
